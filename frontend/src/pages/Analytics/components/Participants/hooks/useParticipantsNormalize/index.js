...
const useParticipantsNormalize = (data) => useMemo(() => {
	if (!_.isEmpty(data)) {
		const names = data.reduce((acc, item) => {
			if (_.get(item, 'participants', 0) <= 0) {
				return [
					...acc
				]
			}
		...
		const series = data.reduce((acc, item, index) => {
			...
			const event = _.times(_.toNumber(_.get(item, 'participants', 0))).reduce((accChild) => {
				if (_.get(item, 'participants', 0) <= 0) {
					return [
						...accChild
					]
				...
				return [
					...accChild,
					{
						...
						data: insert(_.size(_.times(_.toNumber(_.get(item, 'participants', 0)))), null)
					}
				]
			}, [])
...
export default useParticipantsNormalize
